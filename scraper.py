    ...
    if d == today:
        ...
        try:
            ...
            if match:
                ...
                print("【极速解密】未找到混淆解密特征，安全返回空列表。")
                return []
        except Exception as e:
            print(f"【极速解密】出错: {e}，安全返回空列表。")
            return []

    # 对于今日以外的赛事列表拉取，彻底禁用 Playwright 降级，返回空列表（未来赛事由 api-gateway API 接管）
    print(f"Playwright Scraper Disabled for date {date_str} to protect server resources.")
    return []
