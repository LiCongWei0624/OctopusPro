...
HEADERS = {
    'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, Gecko) Version/16.6 Mobile/15E148 Safari/604.1',
...
def fetch_date_n(date_str, n_val):
...
def test():
    ...
    # We query all values of n
    n_values = [1, 2, 3, 4, 5, 7]
    for n in n_values:
        print(f"Fetching n={n} for date {date_str}...")
        data = fetch_date_n(date_str, n)
        if data:
            matches = data.get('matches', [])
            print(f"  n={n} returned {len(matches)} matches")
            for m in matches:
                all_matches[m['id']] = m
        time.sleep(0.5)
        
    print(f"\nCompleted! Total UNIQUE matches found for {date_str}: {len(all_matches)}")
