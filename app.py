...
PREDICTION_POLICY = """这是赛前预测链路。只能依据输入的赛前数据判断：
1. 不得把走地赔率、已结束比分或赛后数据视为赛前证据。
...
def build_match_prompt_context(match_id, home, away):
    ...
    if match_meta:
        context_lines.append(f"- 开赛时间：{match_meta.get('date', '')} {match_meta.get('time', '')}（仅限赛前快照）")
...
def run_ai_analysis_thread(match_id, api_base, api_key, model_name, system_prompt, context_str, ai_cache_file, prediction_metadata):
    global ai_tasks
    try:
...
def match_ai_analysis():
    ...
    # This is a pre-match endpoint.  Rejecting non-pre-match fixtures is the
    # backend authority; a UI-only guard would still allow polluted requests.
    match_status = None
    match_metadata = None
    if os.path.exists(DATA_FILE):
        try:
    ...
    if match_status not in PREMATCH_STATUSES:
        return jsonify({'success': False, 'error': '赛前 AI 预测仅支持未开赛或待定赛事；进行中、已结束、取消或推迟赛事不能生成赛前报告。'})
